...
void Menu::Start(){
	...
	while( ScreenFlip()==0 && ProcessMessage()==0 && ClearDrawScreen()==0 && gpUpdateKey()==0 ){
		...
		// メニュー項目を描画
		LoadGraphScreen(0,20,"Image/title.png",TRUE);
...
string Menu::Select(){
    ...
    MenuElement_t MenuElement[MAX_SELECTION]={
            {  80,100, "Image/stage1.png", "Map/map1.txt"}, //x座標、y座標、Image名、ファイル名の順番で格納される。
			{  50,150, "Image/stage2.png", "Map/map2.txt"},
			{  50,200, "Image/stage3.png", "Map/map3.txt"}
    };
	...
	while( ScreenFlip()==0 && ProcessMessage()==0 && ClearDrawScreen()==0 && gpUpdateKey()==0 ){
		...
		if( key[ KEY_INPUT_DOWN ] == 1 ){ // 下キーが押された瞬間だけ処理
			...
			PlaySoundFile( "Sound/select_sound.wav" , DX_PLAYTYPE_NORMAL ) ;//選択肢がすれたら、選択音を再生
		}
		...
		if( key[ KEY_INPUT_RETURN ] == 1){
			PlaySoundFile( "Sound/decision_sound.wav" , DX_PLAYTYPE_NORMAL ) ;//決定音を再生
			for( int i=0; i<MAX_SELECTION; i++ ){              // メニュー項目数である個数分ループ処理
				if( i == SelectNum ){          // 今処理しているのが、選択番号と同じ要素なら
		...
		// 描画フェーズ
		LoadGraphScreen(20,10,"Image/select_title.png",TRUE);
		...
		for( int i=0; i<MAX_SELECTION; i++ ){ // メニュー項目を描画
			...
			if(i==SelectNum){
				//選択ボタンの画像を表示
				LoadGraphScreen(MenuElement[i].x-30,MenuElement[i].y+10,"Image/selection_key.png",TRUE);
